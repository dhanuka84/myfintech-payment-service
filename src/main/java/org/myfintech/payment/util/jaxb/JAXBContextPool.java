...
/**
 * Pool manager for JAXBContext instances to improve performance.
 * JAXBContext creation is expensive, so we pool and reuse instances.
 */
@Slf4j
@Component
public class JAXBContextPool {
    ...
    public Unmarshaller borrowUnmarshaller(Class<?> clazz) throws Exception {
        GenericObjectPool<Unmarshaller> pool = unmarshallerPools.computeIfAbsent(clazz, 
            k -> createUnmarshallerPool(k));
    ...
    public Marshaller borrowMarshaller(Class<?> clazz) throws Exception {
        GenericObjectPool<Marshaller> pool = marshallerPools.computeIfAbsent(clazz, 
            k -> createMarshallerPool(k));
        return pool.borrowObject();
    }
    ...
    /**
     * Gets or creates a JAXBContext for the given class.
     * Contexts are cached and reused.
     */
